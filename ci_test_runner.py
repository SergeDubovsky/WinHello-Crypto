...
def test_imports():
    ...
    try:
        ...
        print(f"✅ security_config imported (AES_KEY_SIZE={AES_KEY_SIZE})")
        
        from security_utils import ValidationError, SecurityError
        print("✅ security_utils imported")
        
        # Test cryptography imports (may fail on some platforms)
        try:
            from hello_crypto import FileEncryptor, WindowsHelloError
            print("✅ hello_crypto imported")
        except ImportError as e:
            print(f"⚠️  hello_crypto import failed (expected on non-Windows): {e}")
        
        try:
            from aws_hello_creds import AWSCredentialManager
            print("✅ aws_hello_creds imported")
        except ImportError as e:
            print(f"⚠️  aws_hello_creds import failed (expected on non-Windows): {e}")
...
def test_security_functions():
    ...
    try:
        ...
        try:
            ...
            print("✅ AWS credential validation working")
        
        # Test error sanitization
        sanitized = sanitize_error_message("test error", "test operation")
        print(f"✅ Error sanitization working: '{sanitized}'")
...
def test_configuration():
    ...
    try:
        ...
        print("✅ Configuration constants valid")
        return True
...
def main():
    """Run all tests."""
    print("🚀 Starting CI validation tests...")
    print("=" * 50)
    ...
    if passed == total:
        ...
        print("⚠️  Some tests failed (may be expected in CI environment)")
